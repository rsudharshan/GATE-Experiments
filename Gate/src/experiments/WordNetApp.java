...
public class WordNetApp {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      Gate.getCreoleRegister().registerDirectories( new File(Gate.getGateHome().getAbsolutePath() + "/plugins/WordNet").toURI().toURL());
      
      // Create an instance of the plugin. See the user guide for the format of the WordNet property file
      FeatureMap fm = Factory.newFeatureMap();
      fm.put("propertyUrl", new URL("file:///z:/wordnet.xml"));
      WordNet wnMain =
              (WordNet)gate.Factory.createResource("gate.wordnet.JWNLWordNetImpl", fm);
